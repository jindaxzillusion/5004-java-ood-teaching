...
public final class Stevenson {
  ...
  public Stevenson(final double temperature, final double dewPoint, final double windSpeed,
    ...
    if (windSpeed < 0) {
      throw new IllegalArgumentException("wind speed can not be less than 0");
    }
    if (totalRain < 0) {
      throw new IllegalArgumentException("total rain can not be less than 0");
    }
    if (dewPoint > temperature) {
      throw new IllegalArgumentException("dewPoint cannot be greater than temperature");
    }
  ...
  /**
   * Get the exact relative humidity of this weather reading rounded to the nearest integer.
   *
   * @return the exact relative humidity
   */
  private double getExactRelativeHumidity() {
    ...
    double saturatedVaporPressure = 6.11 * Math.pow(10, ((7.5 * temperature)
        / (237.3 + temperature)));
    return actualVaporPressure / saturatedVaporPressure * 100;
  }
  ...
  public int getRelativeHumidity() {
    return (int) Math.round(this.getExactRelativeHumidity());
  }
  ...
  public int getHeatIndex() {
    ...
    double r = getExactRelativeHumidity();
    double heatIndex = c1
        + c2 * temperature
        + c3 * r
        + c4 * temperature * r
  ...
  public int getWindChill() {
    double temperatureFahrenheit = (9.0 / 5.0) * temperature + 32.0;
    double windChill = 35.74
        + (0.6215 * temperatureFahrenheit)
        - (35.75 * Math.pow(windSpeed, 0.16))
        + (0.4275 * temperatureFahrenheit * Math.pow(windSpeed, 0.16));
    double windChillCelsius = (windChill - 32.0) * (5.0 / 9.0);
    return (int) Math.round(windChillCelsius);
  }

  /**
   * Get the String output of Stevenson class.
   *
