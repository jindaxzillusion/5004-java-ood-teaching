...
public class ElementNode implements IListOfBooks {
  ...
  @Override public float totalPrice() {
    return this.book.getPrice() + this.rest.totalPrice();
  }

  @Override public IListOfBooks allBefore(int year) {
    if (book.before(year)) {
  ...
  @Override public IListOfBooks getList(Predicate<Book> test) {
    if (test.test(book)) {
  ...
  @Override public IListOfBooks sortByPrice() {
    return this.rest.sortByPrice().insert(this.book);
  }

  @Override public IListOfBooks insert(Book book) {
